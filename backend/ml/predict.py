...
class FraudPredictor:
    def __init__(self):
        ...
        if model_dir:
            try:
                ...
                # In environments where joblib / sklearn have pickle or version conflicts
                self.model = None
    ...
    def _compute_calibrated_ml_score(self, feats):
        """
        Calibrated mathematical behavioral ML inference engine.
        Ensures continuous, nuanced risk sensitivity even when models are loaded dynamically.
        """
        amt = feats["amount"]
        ...
        # Base log-odds calculation (Generalized Additive Risk Model)
        # 1. Amount baseline log-odds:
        if amt_ratio <= 1.5:
            log_odds = -2.8  # ~6% base risk
        elif amt_ratio <= 3.0:
        ...
        # 2. Device Trust Impact:
        # Unrecognized device is a primary vector for ATO / takeover
        if is_new_dev == 1:
            log_odds += 1.45  # Substantial risk shift

        # 3. Counterparty Familiarity:
        if is_new_rec == 1:
            log_odds += 0.85
            if rec_trust < 50:
                log_odds += 0.75
        else:
            log_odds -= 0.65  # Established receiver discount

        # 4. Temporal / Off-hours:
        if is_night == 1 or unusual_time == 1:
            log_odds += 0.95
            if amt_ratio > 5.0:
                log_odds += 0.60  # Synergy between large amount and off-hours

        # 5. Velocity / Micro-phishing bursts:
        if tx_10m >= 4:
            log_odds += 2.10
        elif tx_10m >= 2:
            log_odds += 1.20
        ...
        # Sigmoid calibration: P(Fraud) = 1 / (1 + exp(-log_odds))
    ...
    def generate_dynamic_reasons(self, feats, ml_prob, risk_score, risk_level):
        """
        Explainable AI: Generates dynamic, context-specific reasons based on exact input features.
        """
        reasons = []
        amt = feats["amount"]
        amt_ratio = feats["amount_to_avg_ratio"]
        ...
        # 3. Amount vs Baseline explanation
        if amt_ratio > 3.0:
        ...
        # 4. Temporal / Time of Day explanation
        ...
        # Fallback for purely normal transactions
        if not reasons:
...
if __name__ == "__main__":
    ...
    test_cases = [
        ("Case 1: Student (Avg 800) -> Mart ₹24,567 at 5:30 AM (Trusted Dev, New Receiver)", {
            "amount": 24567, "sender_account_type": "STUDENT", "sender_avg_amount": 800,
            "transaction_hour": 5, "is_night": 1, "is_new_device": 0, "is_new_receiver": 1
        }),
        ("Case 2: Same at 2:00 PM Daytime", {
            "amount": 24567, "sender_account_type": "STUDENT", "sender_avg_amount": 800,
            "transaction_hour": 14, "is_night": 0, "is_new_device": 0, "is_new_receiver": 1
        }),
        ("Case 3: Same at 5:30 AM with Unrecognized Device", {
            "amount": 24567, "sender_account_type": "STUDENT", "sender_avg_amount": 800,
            "transaction_hour": 5, "is_night": 1, "is_new_device": 1, "is_new_receiver": 1
        }),
    ]
    ...
    for title, payload in test_cases:
        ...
        print("\n" + "="*70)
        print(title)
        print(f"Risk Score: {res['risk_score']}% | Probability: {res['fraud_probability']} | Level: {res['risk_level']} | Auth: {res['authentication']}")
